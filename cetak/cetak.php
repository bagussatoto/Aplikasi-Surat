...
$pdf->Image('../dist/img/logopati1.png',15,18,16);
$pdf->SetTitle("Cetak Data");
$pdf->SetFont('Times','B',14);
$pdf->Cell(0,5,'PEMERINTAH KABUPATEN PATI','0','1','C',false);
$pdf->Cell(0,5,'KECAMATAN GEMBONG','0','1','C',false);
$pdf->SetFont('Times','B',16);
$pdf->Cell(0,5,'DESA KLAKAHKASIHAN','0','1','C',false);
$pdf->SetFont('Times','i',12);
$pdf->Cell(0,5,'Sekretariat : Desa Klakahkasihan RT 03 RW 01 Kode Pos 59162','0','1','C',false);
$pdf->Ln(3);
...
if(@$_GET['data'] == "ket_umum") {
	...
	$pdf->Cell(50,5,'SURAT KETERANGAN / PENGANTAR','0','1','L','U',false);
	$pdf->Cell(50,5,'Nomor : ','0','1','L','U',false);
	$pdf->Ln(3);
	...
	$pdf->Ln(2);
	$no = 1;
	$sql = mysqli_query($koneksi, "SELECT * FROM tb_surat WHERE id_surat='".$_GET['id']."'") or die ($koneksi->error);
	while($data = mysqli_fetch_array($sql)) {
		// if($data['jenis_kelamin'] == 'L') { $jk = "Laki-laki"; } else { $jk = "Perempuan"; }
	$pdf->Ln(4);
	$pdf->SetFont('Times','',8);
	$pdf->Cell(10,4,$no++.".",1,0,'C');
	$pdf->Cell(25,4,$data['id_surat'],1,0,'L');
	...
	$pdf->Ln(2);
	$no = 1;
	$sql = mysqli_query($koneksi, "SELECT * FROM tb_surat WHERE id_surat='".$_GET['id']."'") or die ($db->error);
	while($data = mysqli_fetch_array($sql)) {
		$pdf->Ln(4);
		$pdf->SetFont('Times','',7);
		$pdf->Cell(7,4,$no++.".",1,0,'C');
		$pdf->Cell(14,4,$data['id_surat'],1,0,'L');
...
// 	$sql = mysqli_query($db, "SELECT gaji.tgl, gaji.gaji, karyawan.id_karyawan, karyawan.nama FROM gaji JOIN karyawan ON gaji.id_karyawan=karyawan.id_karyawan WHERE karyawan.nama='".$_POST['kar']."'") or die ($db->error);
